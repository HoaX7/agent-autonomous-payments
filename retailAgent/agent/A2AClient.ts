...
export class A2AClient {
    ...
    private serviceEndpointUrl; // To be populated from AgentCard after fetching
    /**
    ...
    constructor(agentBaseUrl: string) {
        ...
        this.serviceEndpointUrl = agentBaseUrl;
    }
    /**
     ...
     * @returns A Promise that resolves to the AgentCard.
     */
    async getAgentCard(agentBaseUrl: string) {
        if (agentBaseUrl) {
     ...
     */
    async _postRpcRequest(method: string, params: any) {
     ...
     */
    async setTaskPushNotificationConfig(params: any) {
     ...
     */
    async getTaskPushNotificationConfig(params: any) {
     ...
     */
    async updateOrder(id: number, params: any) {
        return fetch(`${this.agentBaseUrl}/updateOrder/${id}`, { method: "POST", body: JSON.stringify(params) })
            .catch(console.error)
    }
    /**
     ...
     * @returns An AsyncGenerator yielding A2AStreamEventData (Message, Task, TaskStatusUpdateEvent, or TaskArtifactUpdateEvent).
     */
    async *resubscribeTask(params: any) {
        const agentCard = await this.agentCardPromise;
        if (!agentCard.capabilities?.streaming) {
     ...
     */
    async *_parseA2ASseStream(response: Response, originalRequestId: number) {
        if (!response.body) {
     ...
     */
    _processSseEventData(jsonData: string, originalRequestId: any) {
        if (!jsonData.trim()) {
